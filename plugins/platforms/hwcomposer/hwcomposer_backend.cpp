...
KWayland::Server::OutputInterface *HwcomposerBackend::hwc2_createOutput()
{
    HWC2DisplayConfig *config = hwc2_compat_display_get_active_config(m_hwc2_primary_display);
    assert(config);

    QSize pixel(config->width, config->height);
    if (pixel.isEmpty())
    {
        return nullptr;
    }
    ...
    using namespace KWayland::Server;
    OutputInterface *o = waylandServer()->display()->createOutput(waylandServer()->display());
    o->addMode(pixel, OutputInterface::ModeFlag::Current | OutputInterface::ModeFlag::Preferred, (config->vsyncPeriod == 0) ? 60000 : 10E11 / config->vsyncPeriod);

    if (config->dpiX != 0 && config->dpiY != 0)
    {
        static const qreal factor = 25.4;
        m_physicalSize = QSizeF(qreal(pixel.width() * 1000) / qreal(config->dpiX) * factor,
                                qreal(pixel.height() * 1000) / qreal(config->dpiY) * factor);
        o->setPhysicalSize(m_physicalSize.toSize());
    }
    else
    {
...
void HwcomposerBackend::init()
{
    ...
    if (m_hwcVersion == HWC_DEVICE_API_VERSION_2_0)
        output = hwc2_createOutput();
    else 
        output = createOutput(hwcDevice);
